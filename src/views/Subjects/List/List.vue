    ...
    <b-card no-body>
        ...
        <el-table class="table-responsive table"
            ...
            <el-table-column label="Descrição"
                            prop="description">
                <template v-slot="{row}">
                    <span class="font-weight-600 text-truncate mb-0 text-sm">{{row.description}}</span>
                </template>
            </el-table-column>
            ...
            <div>
              <b-form @submit.stop.prevent="onSubmit">
                <b-form-group label="Descrição" label-for="description">
                  ...
                  >
                    A descrição do assunto é obrigatória e pode conter no máximo 40 caracteres.
                  </b-form-invalid-feedback>
                </b-form-group>
                <div class="row">
...
<script>
  ...
  export default {
    ...
    methods: {
      loadData() {
        axios.get('subjects').then(response => (this.subjects = response.data.data))
      },
      validateState(name) {
      ...
      onSubmit() {
        ...
        const path = form.id > 0 ? `subjects/${form.id}` : 'subjects'
      ...
      new() {
        ...
        this.editingTitle = "Novo Assunto";
        this.$bvModal.show('form');
      },
      edit(row) {
        this.form = row
        this.editingTitle = `Editando "${row.description}"`
        this.$bvModal.show("form")
      },
      hideModal() {
          this.form = {}
      ...
      confirmDelete(row) {
        this.$bvModal.msgBoxConfirm(`Deseja realmente remover o Assunto "${row.description}"?`, {
            title: 'Tem certeza?',
            size: 'sm',
            buttonSize: 'sm',
            okVariant: 'danger',
        ...
        .then(value => {
          if (value) {
            axios.delete(`subjects/${row.id}`).then(response => {
              this.makeAlert('info', 'Assunto excluído com sucesso!')
              this.loadData()
            })
          }
        })
